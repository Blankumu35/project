...
{
    // Compute local transform of this node from its TRS (or matrix)
    const tinygltf::Node &node = model.nodes[nodeIndex];
        ...
        std::vector<glm::mat4> &nodeTransforms) 
{
    ...
    if (model.animations.size() > 0 && model.skins.size() > 0) {
        const tinygltf::Animation &animation = model.animations[0];
        const AnimationObject &animationObject = animationObjects[0];

        // Local node transforms, start from identity and let the animation channels fill them
        ...
        // Recompute global transforms from the scene roots
        ...
        for (size_t i = 0; i < scene.nodes.size(); ++i) {
            ...
            computeGlobalNodeTransform(model, nodeTransforms, rootIndex, identity, globalNodeTransforms);
        }

        // Store for rendering (Skeleton::renderSkeleton uses these)
